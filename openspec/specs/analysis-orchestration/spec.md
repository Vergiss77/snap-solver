...
分析流程 SHALL 将截图图像发送给配置的多模态 LLM，要求其判断图像内容是否为题目；若是题目，MUST 分类为选择题、填空题、简答题或编程题之一，并生成对应的解答与解题思路；若为编程题，解答 MUST 包含可运行的代码。

#### Scenario: 识别为选择题

- **WHEN** LLM 判定截图为选择题
- **THEN** 分析结果包含题型标记"选择题"、正确选项及逐项解析思路

#### Scenario: 识别为编程题

- **WHEN** LLM 判定截图为编程题
- **THEN** 分析结果包含题型标记"编程题"、解题思路与完整代码实现
